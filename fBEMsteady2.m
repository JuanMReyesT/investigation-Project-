...
function [RES] = fBEMsteady2(WT ,Sim ,Wind , Algo)
...
[CLp,CDp,CLpn,CDpn]=prueba2(Re);
 alp=alpha*180/pi;
 alpha_st=10*pi/180;
 Clstall=polyval(CLp,alp);
 Cdstall=polyval(CDp,alp);
 if alp>=10
...
function [a,aprime] = fInductionCoefficients( a_last ,Ct ,Cq ,F,lambda_r , varargin) 
